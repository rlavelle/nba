"""
chosen using forward selection on a simple regression model using BIC stopping criterion
"""

# FS_POINTS = [
#     'points_points_cum_ssn_ema_bayes_post',
#     'fieldGoalsAttempted_ema_5',
#     'foulsDrawn_ema_7',
#     'fieldGoalsAttempted_fieldGoalsAttempted_cum_ssn_avg_bayes_post',
#     'minutes_1g',
#     'possessions_cum_ssn_ema',
#     'fieldGoalsMade_cum_ssn_avg',
#     'fieldGoalsMade_cum_ssn_ema',
#     'usagePercentage_ema_3',
#     'ppm_cum_ssn_ema',
#     'fieldGoalsAttempted_cum_ssn_avg',
#     'fieldGoalsMade_fieldGoalsMade_cum_ssn_ema_bayes_post',
#     'estimatedUsagePercentage_ema_3',
#     'freeThrowsPercentage_ema_3',
#     'minutes_cum_ssn_ema',
#     'points_cum_ssn_avg',
#     'points_cum_ssn_ema',
#     'pointsPaint_sma_5',
#     'possessions_sma_3',
#     'usagePercentage_sma_3',
#     'fieldGoalsAttempted_ema_7',
#     'minutes_ema_7',
#     'possessions_ema_5',
#     'percentagePersonalFoulsDrawn_sma_5',
#     'usagePercentage_ema_7',
#     'PIE_ema_7',
#     'minutes_sma_3'
# ]
FS_POINTS = [
    'points_points_cum_ssn_ema_bayes_post',
    'fieldGoalsAttempted_ema_5',
    'foulsDrawn_ema_7',
    'minutes_1g_v_minutes_cum_ssn_ema_hot_streak',
    'fieldGoalsAttempted_fieldGoalsAttempted_cum_ssn_avg_bayes_post',
    'fieldGoalsAttempted_fieldGoalsAttempted_cum_ssn_ema_bayes_post',
    'points_cum_ssn_ema',
    'points_cum_ssn_avg',
    'usagePercentage_ema_3',
]

# found from forward selection on regression
# FS_MINUTES = [
#     'minutes_ema_7',
#     'minutes_minutes_cum_ssn_avg_bayes_post',
#     'minutes_1g',
#     'possessions_3g_v_possessions_ema_5_hot_streak',
#     'ppm_1g',
#     'estimatedPace_sma_3',
#     'estimatedPace_1g',
#     'estimatedPace_3g_v_estimatedPace_cum_ssn_avg_hot_streak',
#     'pointsOffTurnovers_ema_7',
#     'minutes_sma_3',
#     'possessions_cum_ssn_avg',
#     'oppPointsOffTurnovers_3g_v_oppPointsOffTurnovers_cum_ssn_avg_hot_streak',
#     'oppPointsSecondChance_1g',
#     'points_1g',
#     'possessions_ema_7',
#     'possessions_1g',
#     'estimatedPace_ema_7',
#     'possessions_sma_3',
#     'minutes_cum_ssn_avg',
#     'freeThrowsPercentage_ema_3',
#     'oppPointsSecondChance_sma_3'
# ]
FS_MINUTES = [
    'minutes_ema_7',
    'minutes_minutes_cum_ssn_avg_bayes_post',
    'minutes_1g_v_minutes_sma_5_hot_streak',
    'possessions_3g_v_possessions_ema_5_hot_streak',
    'usagePercentage_1g_v_usagePercentage_ema_5_hot_streak',
    'pointsOffTurnovers_ema_7',
    'estimatedPace_ema_7',
    'possessions_3g_v_possessions_sma_5_hot_streak',
    'estimatedPace_1g_v_estimatedPace_cum_ssn_avg_hot_streak',
    'trueShootingPercentage_1g_v_trueShootingPercentage_cum_ssn_ema_hot_streak',
    'oppPointsOffTurnovers_3g_v_oppPointsOffTurnovers_cum_ssn_avg_hot_streak',
    'oppPointsSecondChance_1g_v_oppPointsSecondChance_ema_3_hot_streak',
    'possessions_ema_7',
]
