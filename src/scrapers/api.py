...
class API:
    ...
    def _get_retry(self,
                   ...
                   reset_session_per_retry: bool = False) -> dict[str]:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                # Reset session on timeout if needed
                if reset_session_per_retry and use_session:
                    session = requests.Session()
                    if headers:
                        session.headers.update(headers)
    ...
    def _get(self,
             ...
             cache: bool = False,
             use_session: bool = False,
             reset_session_per_retry: bool = False) -> Union[dict[str], Any]:

        if not cache:
            return self._get_retry(url=url,
                                   params=params,
                                   headers=headers,
                                   use_session=use_session,
                                   reset_session_per_retry=reset_session_per_retry)
        ...
        response = self._get_retry(url=url,
                                   params=params,
                                   headers=headers,
                                   use_session=use_session,
                                   reset_session_per_retry=reset_session_per_retry)
